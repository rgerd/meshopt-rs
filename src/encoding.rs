...
/// Like `encode_vertex_buffer`, but with explicit `vertex_count` and `vertex_size`
/// for buffers where `size_of::<T>()` does not equal the logical vertex stride.
///
/// This is useful when working with raw byte buffers (`&[u8]`) or packed element buffers
/// (`&[u16]`) where the Rust element type does not correspond to a full vertex.
pub fn encode_vertex_buffer_sized<T>(
    vertices: &[T],
    vertex_count: usize,
    vertex_size: usize,
) -> Result<Vec<u8>> {
